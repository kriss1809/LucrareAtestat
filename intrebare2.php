...
<body style="background-color: mistyrose ">
    ...
    <?php
        ...
        if(isset($limba) && $limba=="eng")
            $q = "SELECT * FROM `quiz2-en` WHERE nr='{$_SESSION['contor2']}'";
        if(isset($limba) && $limba=="es")
            $q = "SELECT * FROM `quiz2-sp` WHERE nr='{$_SESSION['contor2']}'";
        if(isset($limba) && $limba=="fr")
            $q = "SELECT * FROM `quiz2-fr` WHERE nr='{$_SESSION['contor2']}'";
        $rez = mysqli_query($link , $q);
        while($L = mysqli_fetch_assoc($rez))
        {
            ?>
            <div class="container mt-5 p-3 col-9" style="background-image: linear-gradient(to bottom right, #c7b0de, #e6dbf0); border-radius: 21px;">
                <form method="get" action="verificare2.php?nr=<?=$_SESSION['contor2']?>">

                <h3 class="pt-3"><?=$L['nr']?>. <?=$L['intrebare']?> </h3>
                <div class="row p-2">
                    <div class="col-4">
                    </div>
