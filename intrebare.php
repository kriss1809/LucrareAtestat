...
<body style="background-color: mistyrose ">
    ...
    <?php
        ...
        if(isset($limba) && $limba=="eng")
            $q = "SELECT * FROM `quiz-en` WHERE nr='{$_SESSION['contor']}'";
        if(isset($limba) && $limba=="es")
            $q = "SELECT * FROM `quiz-sp` WHERE nr='{$_SESSION['contor']}'";
        if(isset($limba) && $limba=="fr")
            $q = "SELECT * FROM `quiz-fr` WHERE nr='{$_SESSION['contor']}'";
        ...
        while($L = mysqli_fetch_assoc($rez))
        {
            ...
            <div class="container mt-5 p-3 col-9" style="background-image: linear-gradient(to bottom right, LightSteelBlue, PaleTurquoise); border-radius: 21px;">
                <form method="get" action="verificare.php?nr=<?=$_SESSION['contor']?>">
